...
def create_index(
    ...
    imgs, labels = create_image_label(f"data/splits/{name}/{txt}.txt")
    index = []
    for ind, img in enumerate(imgs):
        path = os.path.join(datasets_path[name], imgs[ind])
        index.append([path, labels[ind]])
...
class MixedDataset(Dataset):
    def __init__(self, target_dataset, source_dataset, TS_mapping, transform, num_class, mixup_alpha, mode="RGB"):
    ...
    def __getitem__(self, index):
        #prepare mixed data
        target_path, target_label = self.target_dataset[index]

        if self.TS_mapping is None:
            source_idx = random.randint(0, len(self.source_dataset)-1)
        else:
            source_idx = self.TS_mapping[index]
        source_path, source_label = self.source_dataset[source_idx]
        
        x1 = self.load_image(target_path)
        x2 = self.load_image(source_path)

        l = np.random.beta(self.mixup_alpha, self.mixup_alpha)
        l = max(l, 1 - l)

        y1 = target_label.long()
        y2 = source_label.long()
        
        y1 = torch.zeros(self.class_num).scatter_(0, y1.cpu(), 1).float()
        y2 = torch.zeros(self.class_num).scatter_(0, y2.cpu(), 1).float()
