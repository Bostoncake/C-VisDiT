...
# used in mixup
def euclidean(feat1, feat2):
    bs1 = feat1.size()[0]
    bs2 = feat2.size()[0]
    x11 = torch.sum(torch.pow(feat1, 2), 1).view(bs1, 1).repeat(1, bs2)
    x12 = torch.matmul(feat1, feat2.transpose(0, 1))
    x22 = torch.sum(torch.pow(feat2, 2), 1).view(1, bs2).repeat(bs1, 1)
    x = x11 - 2 * x12 + x22
    return x